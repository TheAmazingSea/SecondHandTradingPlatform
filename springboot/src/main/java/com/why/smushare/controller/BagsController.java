...
public class BagsController {
    ...
    @Resource
    BaseMapper bagsMapper;


    //新增衣物
    @PostMapping
    //@RequestBody就是把前台传过来的json数据转换成Java对象
    public Result save(@RequestBody Bags bags){
        bags.setAllow("0");
    ...
    @PutMapping
    //@RequestBody就是把前台传过来的json数据转换成Java对象
    public Result update(@RequestBody Bags bags){
        bags.setAllow("0");
        bagsMapper.updateById(bags);
        return Result.success();
    }
    ...
    public Result<?> findPage(@RequestParam(defaultValue = "1") Integer pageNum,
        ...
        QueryWrapper<Bags> wrapper = new QueryWrapper<>();
        wrapper.like("name",search);
        wrapper.eq("allow","1");
        return Result.success(bagsMapper.selectPage(new Page<>(pageNum, pageSize),wrapper));
    }
